...
class scope_tree_stringizer {
    ...
    template<class Range>
    std::string visit_symbols(Range const& symbols, size_t const i) const
    {
        if (boost::empty(symbols)) {
            return "";
        }
        return boost::accumulate(symbols | transformed([this, i](auto const& s){ return indent(i) + s->name; }),
                          std::string{},
                          [](auto const& acc, auto const& item){
                              return acc + '\n' + item;
                          });
...
public:
    ...
    std::string visit(scope::local_scope const& l, size_t const i) const
    {
        return indent(i) + "LOCAL_SCOPE"
            ...
            + visit_symbols(l->local_vars, i+1);
    }
