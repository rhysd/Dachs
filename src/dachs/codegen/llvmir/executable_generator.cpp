    ...
    std::string generate_executable(std::vector<std::string> const& libdirs, String const parent_dir_path)
    {
        ...
        auto command
            = os_type == llvm::Triple::Darwin
                ? "ld -macosx_version_min 10.9.0 \"" + objs_string + "\" -o \"" + executable_name + "\" -lSystem -ldachs-runtime -lgc -L /usr/lib -L /usr/local/lib -L " DACHS_INSTALL_PREFIX "/lib -L '" DACHS_LIBGC_PATH "/lib'"
                : (DACHS_CXX_COMPILER " ") + objs_string + " -o " + executable_name + " -ldachs-runtime -lgc -L /usr/lib -L /usr/local/lib -L " DACHS_INSTALL_PREFIX "/lib -L '" DACHS_LIBGC_PATH "/lib'"; // Fallback...
