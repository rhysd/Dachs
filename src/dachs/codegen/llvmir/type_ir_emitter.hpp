...
public:
    ...
    // Note:
    // return T* because variable length array is generated by alloca and llvm.stacksave() and llvm.stackrestore()
    llvm::PointerType *emit(type::array_type const&)
    {
        throw not_implemented_error{__FILE__, __func__, __LINE__, "variable array type LLVM IR generation"};
    }

    // Note:
    // Array type for array literals
    llvm::ArrayType *emit(type::array_type const& a, std::size_t const size)
    {
        return llvm::ArrayType::get(emit(a->element_type), size);
    }
